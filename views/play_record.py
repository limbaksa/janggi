        ...
        if not int(params.id):
            ...
            table=ft.DataTable(columns=[
                        ...
                        rows=[ft.DataRow(cells=[
                            ...
                            ft.DataCell(ft.IconButton(icon=ft.icons.PLAY_CIRCLE_FILL_OUTLINED,data=i,on_click=open_record)),
            ...
            def eval(e):
                ...
                for i in range(1,len(board.fenRecord)):
                    eval_list.append(new_get_eval(board.fenRecord[i]))
            ...
            prev_move=ft.ElevatedButton(content=ft.Text("이전 수"),on_click=board.undo_move)
            ...
            table=ft.DataTable(columns=[
                        ...
                        rows=[ft.DataRow(cells=[
                            ft.DataCell(ft.Text(f"{board.moveNotation[2*i]}"),data=2*i,on_tap=set_move),
                            ft.DataCell(ft.Text(f"{self.eval_values[params.id][2*i] if params.id in self.eval_values else '-'}")),
                            ft.DataCell(ft.Text(f"{'-' if 2*i+1>=len(board.moveNotation) else board.moveNotation[2*i+1]}"),data=2*i+1,on_tap=set_move),
                            ft.DataCell(ft.Text(f"{self.eval_values[params.id][2*i+1] if (params.id in self.eval_values and 2*i+1<len(board.moveNotation)) else '-'}")),
                        ]) for i in range((len(board.moveNotation)+1)//2)]
                        )

            lv = ft.ListView(expand=1, spacing=10, padding=20)
            lv.controls.append(table)
            return ft.View(f'/record/{params.id}',controls=[
                ft.Row([
                        ft.NavigationRail(
                            selected_index=None,
                            label_type=ft.NavigationRailLabelType.ALL,
                        ...
                        ft.VerticalDivider(width=1),
                        board,
                        ft.Column(controls=[next_move,prev_move,reset,eval_all],alignment=ft.MainAxisAlignment.CENTER,height=600),
                        ft.VerticalDivider(width=1),
                        lv
                    ],expand=True)
            ])