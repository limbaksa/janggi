...
class janggiPiece(ft.GestureDetector):
    ...
    def start_drag(self, e: ft.DragStartEvent):
        self.move_on_top()
        if self.piece.color == self.board.board.turn and (
            not self.board.ai or self.piece.color != self.board.aiturn
        ):
    ...
    def drop(self, e: ft.DragEndEvent):
        ...
        for piece in self.board.piecelist:
            piece.move_on_top()
        if self.piece.color != self.board.board.turn or (
            self.board.ai and self.piece.color == self.board.aiturn
        ):
            self.bounce_back()
            self.update()
            return
        for slot in self.board.slots:
            if abs(self.top - slot.top) < 30 and abs(self.left - slot.left) < 30:
                if self.piece.isValidMove(self.board.slots.index(slot)):
                    trymove = self.board.board.move(
                        self.piece.location, self.board.slots.index(slot)
                    )
                    if trymove:
                        self.place(slot)
                        self.update()
                        if trymove == -1:
                            self.board.gameOver(
                                self.board.board.isGameOver(self.board.board.turn)
                            )
