...
class Piece:
    ...
    def isAttacking(self,dest:int)->bool:
        return self.board.pieceColor(dest)!=self
    ...
    def __str__(self)->str:
        return self.__class__.__name__[0] if self.color==0 else self.__class__.__name__[0].lower()
        ...
        for i in range(2):
            if start in palaceDiagonal[i] and dest==palaceCenter[i]:# 대각선->중심
                ...
                if dest-start in [-10,-1,1,10]:
                    return True
            if (isinstance(self,Chariot) or isinstance(self,Cannon)) and start in palaceDiagonal[i] and dest in palaceDiagonal[i]:# 차,포의 움직임
...
class Soldier(SlidingPiece):
    ...
    def isAttacking(self, dest: int) -> bool:
        ...
        if [dest%10-self.location%10,dest//10-self.location//10] not in [[1,0],[-1,0],[0,1],[0,-1]] and not self.palaceMove(self.location,dest):
            return False
...
class Chariot(SlidingPiece):
    ...
    def isAttacking(self, dest: int) -> bool:
        if (self.location//10)==(dest//10) or (self.location%10)==(dest%10) or self.palaceMove(self.location,dest):
            if self.board.piecesBetween(self.location,dest)==0 and self.board.pieceColor(dest)!=self.color:
                return True        
        return False
...
class Cannon(SlidingPiece):
    ...
    def isAttacking(self, dest: int) -> bool:
        if (self.location//10)==(dest//10) or (self.location%10)==(dest%10) or self.palaceMove(self.location,dest):
            if self.board.piecesBetween(self.location,dest)==1 and self.board.pieceColor(dest)!=self.color:
                if self.location//10==dest//10:
                    ...
                    if (not isinstance(self.board.boardState[(self.location+dest)//2],Cannon)) and (not isinstance(self.board.boardState[dest],Cannon)):
                        return True
                    else:
                        return False
                return True
...
class Horse(Piece):
    ...
    def isAttacking(self, dest: int) -> bool:
        if [dest%10-self.location%10,dest//10-self.location//10] not in [[2,1],[2,-1],[1,2],[1,-2],[-2,1],[-2,-1],[-1,2],[-1,-2]]:
            return False
        if self.board.piecesBetween(self.location,dest)==0 and self.board.pieceColor(dest)!=self.color:
            return True
        return False
...
class Elephant(Piece):
    ...
    def isAttacking(self, dest: int) -> bool:
        if [dest%10-self.location%10,dest//10-self.location//10] not in [[3,2],[3,-2],[2,3],[2,-3],[-3,2],[-3,-2],[-2,3],[-2,-3]]:
            return False
        if self.board.piecesBetween(self.location,dest)==0 and self.board.pieceColor(dest)!=self.color:
            return True
        return False
...
class Board:
    ...
    def isBikjang(self):
        if self.wasBikjang and (self.kings[0].location//10==self.kings[1].location//10 or self.kings[0].location%10 ==self.kings[1].location%10) and self.piecesBetween(self.kings[0].location,self.kings[1].location)==0:
            return True
        return False
        ...
        if self.boardState[start]:
            if self.boardState[dest]:
                for i in range(len(self.pieces[self.boardState[dest].color])):
                    if self.pieces[self.boardState[dest].color][i] is self.boardState[dest]:
